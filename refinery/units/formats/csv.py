...
def json_to_csv(table: list | dict, quote: str = '"', delim: str = ',', codec: str = 'utf8'):
    """
    Convert a JSON structure to CSV.
    """
    ...
    with io.TextIOWrapper(out, codec, newline='') as stream:
...
class csv(Unit):
    ...
    def reverse(self, data: bytearray):
        ...
        return json_to_csv(
            table,
            self.args.quote.decode(self.codec),
            self.args.delim.decode(self.codec),
            self.codec,
        )
