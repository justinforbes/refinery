...
_OPERATORS = _load('pwsh-operators.json.gz')
_operator_schema = _OPERATORS['schema']['version']
if _operator_schema != SCHEMA_VERSION:
    raise ValueError(
        F'pwsh operator grid schema version {_operator_schema} is not the expected '
        F'{SCHEMA_VERSION}; the reader and the collected data are out of step.'
    )
...
def binary_outcome(
    ...
    *nothing is known here* and decline, never as *this produces nothing*.
    """
    by_left = _OPERATORS['binary'].get(operator.lower())
    if by_left is None:
        return None
...
def _grid_lookup(by_left: dict, left, right) -> list[str] | None:
    left_key = _grid_key(left)
    right_key = _grid_key(right)
    if left_key is None or right_key is None:
        return None
    by_right = by_left.get(left_key)
    return None if by_right is None else by_right.get(right_key)
