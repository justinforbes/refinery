...
This is out of contract for the same reason self-inspecting code is on every other language here:
an analysis that rewrites a script cannot also promise to preserve a script that re-runs its own
text. Every ordinary re-entry — a loop, a re-entered function body, a `process` block — is modelled
or refused; only genuine self-re-invocation is excluded, and it is stated rather than left silent.
"""
...
def build_world_reach(
    ...
    `process` block that re-runs) or an opener cannot be placed in the root graph. Each is the
    fail-closed direction the module docstring states: the flood cannot bound where such an opener
    ran, so no read is granted over it.
    """
    ...
    refuse = root.process_block is not None
