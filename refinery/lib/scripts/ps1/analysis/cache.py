...
the script being transformed and shares it across every transform in a run, rebuilding the model only
after that script's tree changes — whether a transform announces the change through
...
class Ps1ModelCache(ModelCacheBase):
    """
    Lazily builds and memoizes the `refinery.lib.scripts.ps1.analysis.model.Ps1SemanticModel` for one
    root script, dropping it whenever this root's AST-mutation counter advances past the value it was
    built at. Later phases add `effects` and `control_flow` slots behind the same shape.
    """
...
def model_cache(transformer: Transformer, root: Ps1Script) -> Ps1ModelCache:
    """
    The pipeline's shared `Ps1ModelCache` for *root* when one is attached to *transformer* and built
    over that same root, otherwise a fresh cache stashed back onto *transformer* for reuse within its
